...
class Bert(Model):
    ...
    def init_device_placement(self):
        ...
        if self.config.task == "SQUAD" or self.config.task == "NER":
    ...
    def build_graph(self, indices, positions, segments, masks=None):
        ...
        print("Bert Model", self.config.task)
        # Fine Tuning tasks
        if self.config.task == "NER":
            with self.squad_scope:
                squad_outputs = self.ner_projection(x)
    ...
    def attention_mask(self, masks, data):
        """
        ...
        with mask_scope:
            if not self.config.use_packed_sequence_format:
                ...
                input_mask = self.builder.reshape_const(self.builder.aiOnnx,[masks[0]],
    ...
    def attention_onnx(self, qkv, masks):
        ...
        with self.builder.nameScope('Z'):
            ...
            if not self.config.no_mask or masks is not None:
                mask = self.attention_mask(masks, x)
    ...
    def ner_projection(self, input_x):
        classes = 9
        weight = self.normal_init_tensor(self.config.dtype,
                                         [self.config.hidden_size, classes],
                                         0, 0.02,
                                         "NerW")
        bias = self.constant_init_tensor(self.config.dtype, (classes,), 0, "NerB")
        x = self.builder.aiOnnx.gemm([input_x, weight, bias])
        return [x]
