...
def generate_initializers(config, map_names, load_data, mapping, transform={}, inference=False):
    """
    ...
    import pickle
    with open('tf_data.pkl','wb') as fp:
        pickle.dump(initializers, fp)
    return initializers
...
def load_initializers_from_tf(
    ...
    return generate_initializers(config, names, data, mapping, transform, inference)
...
def load_model_from_tf(
    ...
    """
    initializers = load_initializers_from_tf(file_path, is_checkpoint, config, task, inference)
