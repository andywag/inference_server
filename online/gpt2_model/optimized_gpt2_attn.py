...
class OptimizedGPT2Attention(GPT2Attention):
    def optimized_attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if not self.is_cross_attention:
            ...
            causal_mask = torch.tensor(-10000 * (1 - np.tril(np.ones((query_length, key_length)), -1)),
                                       requires_grad=False, dtype=torch.float32)
            causal_mask = causal_mask.expand_as(attn_weights)
            causal_mask, *_ = poptorch.custom_op(
                [causal_mask],
                "TensorRemap",
                "custom.ops",
                1,
                example_outputs=[causal_mask],
...
class OptimizedGPT2Attention_test(GPT2Attention):
...
class OptimizedGPT2Attention_nobuffer(GPT2Attention):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        super().__init__(config, is_cross_attention=False, layer_idx=None)
        self.config = config
