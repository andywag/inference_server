...
import copy
from heap_utils_triton import CircularInputBuffer, PackingQueueHolder, CircularOutputBuffer
...
class BertInterfaceWrapper():
    def __init__(self, 
        ...
        self.blocks = PackingQueueHolder(self.b, self.s, internal_buffer_width=internal_buffer_width, input_queue_size=input_queue_size, 
    ...
    def run_ipu(self):
        ...
        while self.run_enable:
            ...
            runtime = time.time()
            print("Running Time", runtime - tic, self.blocks.run_block_queue.qsize())
    ...
    def output_results(self, input_queue, block_size, hidden):
        while True:
            ...
            #print("Output Results", self.run_enable, input_queue.qsize())
            if self.run_enable:
                if self.ner:
