...
class PackingQueueHolder:
    ...
    def insert(self, input_ids, segment_ids, id, sender_id, flush = False):
        #print("Inserting Data", self.waiting, self.final_queue.qsize())
        ...
        # FIXME : Queue statement meant to handle lockups. Need better sync method
        if self.waiting or self.run_block_queue.qsize() <= 1:
            self.run_block_queue.put(self.working)
            self.working = Block(self.b, self.h, self.index)
            self.index += 1

        #print("Inserting Data", len(self.working.specs))

        return (True, False, self.index)

    def insert_block(self, transfer):
        input_ids = transfer[0][0]
        segment_ids = transfer[0][1] 
        ids = transfer[0][2]
